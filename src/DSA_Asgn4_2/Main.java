...
//            posInFirst = first.ad
    ...
    public boolean possibleToReturn(Vertex vertex) {
        int visitedCount = 0;
        vertex.setVisited(true);
        while(vertex.getAdjVertices().hasNext()) {
            Vertex next = (Vertex) vertex.getAdjVertices().next();
            possibleToReturnRecursive(next, vertex);
        }
        return false;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            fos = new FileOutputStream("able.txt");
        ...
        try {
            ...
            if (myGraph.isConnective())
                bw.write("yes ");
            else
                bw.write("no ");

            myGraph.removeEdge(interestingEdge);
