...
//            hidden.appendSubtreeToLeft(tmp, hidden.getLeftChild(currentNode));
        ...
        public E getRightChild(Position<E> position) {
            return hidden.getRightChild(position) != null ? hidden.getRightChild(position).getElement() : null;
        }
    }

    public static void main(String[] args) {
        MyLinkedBinarySearchTree<Integer> myLinkedBinarySearchTree = new MyLinkedBinarySearchTree<>();
        InputStream is = null;
        ...
        try {
            is = new FileInputStream("bst.in");
            isr = new InputStreamReader(is, Charset.forName("UTF-8"));
            br = new BufferedReader(isr);
            fos = new FileOutputStream("bst.out");
        ...
        try {
            ...
            for (String word: line.split(" ")) {
                myLinkedBinarySearchTree.add(Integer.parseInt(word));
            }

            line = br.readLine();
            for (String word: line.split(" ")) {
                try {
                    myLinkedBinarySearchTree.delete(Integer.parseInt(word));
            ...
            line = br.readLine();
            for (String word: line.split(" ")) {
                Position<Integer> p = myLinkedBinarySearchTree.find(Integer.parseInt(word));
                Integer c = null;
                if (p != null)
                   c = myLinkedBinarySearchTree.getRightChild(p);
                if (c != null)
                    bw.write(c + " ");
                else
                    bw.write("null ");
